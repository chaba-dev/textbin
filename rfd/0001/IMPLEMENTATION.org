...
- [X] Admin list queries are paginated, scoped in SQL, and do not load paste bodies.
- [X] General recent-paste discovery exposes only public pastes, and metadata views
...
- [X] Sensitive actions enforce the documented reason and recent-reauthentication
  matrix.
- [X] Once the RFD 4 report model exists, platform administrators can page through
...
- [X] Authorization, role-change invalidation, final-admin concurrency,
